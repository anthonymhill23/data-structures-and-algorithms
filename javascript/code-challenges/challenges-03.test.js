...
const containsAnd = (arr) => {
  // Solution code here...
  const resultArray = arr.filter(string => string.includes ('and'));
  return resultArray
};
...
const oddValues = (arr) => {
  const odValues = arr.filter(num =>(num % 2 === 1)
)
...
const notInFirstArray = (forbiddenValues, arr) => {
  const forVal = arr.filter(value => !forbiddenValues.includes(value));
  // Solution code here...
  return forVal
};
