...
const firstLetters = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++){
    let firstLetter = arr[i].match(/\b(\w)/g);
    tempArray.push(firstLetter);
    merged = [].concat.apply([], tempArray);
  }
...
const standardizePhoneNumbers = (arr) => {
  ...
  for (let i = 0; i < arr.length; i +=1){
    nums = arr[i].match(reg);
    newFormatString = nums.join('');
    newFormatArray.push(newFormatString);
  }
...
const onlyOddChars = (str) => {
  ...
  for (let i = 1; i < str.length; i+=2){
    oddIndexString = oddIndexString + str.substring(i, i+1);
  }

  return oddIndexString;
